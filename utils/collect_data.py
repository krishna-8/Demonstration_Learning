...
def _setupTask(phase, env, robot, current):
    ...
    for i, wp in enumerate(waypoints):
        if wp[0] == "L": # Move linear in tool space
            ...
            # print(part)
            grasp_active = True
        elif wp[0] == "R": # Releasing thr gripper
            # trajectory = np.tile(trajectory[-1,:],reps=[30,1])
            part[:,-1] = 1.0
            grasp_active = False
...
def collectSingleSample(pyrep, counter):
    robot = Robot()
        ...
        if frame == 0:
            ...
            try:
                ...
                if counter < 15:
                    ...
                    saveTaskToFile(DATA_PATH2 + name, task)
                task     = None
                trj_step = 0
                if phase == 2:
                    done = True
